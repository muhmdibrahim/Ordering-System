...
def get_products(request):
    ...
    return render(request, 'index.html', {'products': products, 'user': request.user.user})
@login_required
def add_product(request):
    if request.method == 'POST':
        form = ProductForm(request.POST)
        if request.user.user.role != 'operator':
            return HttpResponse("You are viewer, You are not authorized to create an order.", status=403)
        else:
            if form.is_valid():
                product = form.save(commit=False)
                product.created_by = request.user.user
                product.company = request.user.user.company
                product.save()
                return HttpResponse("Product added successfully.", status=201)
            else:
                form = ProductForm()
        products = Product.objects.filter(is_active=True)
        return render(request, 'index.html', {'form': form, 'products': products, 'user': request.user.user})
@login_required
def create_order(request):
    if request.method == 'POST':
        ...
        orders = order.objects.all()
        orders = orders.filter(created_by=request.user.user)
        action = 'viewed'
        return render(request, 'orders.html', {'orders': orders, 'action': action})
@login_required 
def delete_product(request, product_id):
    product = get_object_or_404(Product, id=product_id)
    if product.created_by.user == request.user and request.user.user.role == 'operator':
